...
export function generateInvoiceFormEmail(recipientName, formLink) {
    const safeName = escapeHtml(recipientName || 'there');
    return renderAuthEmail({
        eyebrow: "Invoice Request",
        title: `Complete Your <span class="highlight">Invoice</span>`,
        body: `<p class="text">Hello ${safeName}, we need your billing information to process your payment. Please fill out the form using the button below.</p>`,
        ctaLabel: "Fill Invoice Form",
        ctaLink: formLink,
        footerNote: `<p class="text" style="font-size:12px; color:#6b7280;">This link will expire in 30 days. If you have any questions, please contact us.</p>`,
    });
}

export function generateInvoiceReminderEmail(recipientName, invoiceNumber, daysLeft) {
    const safeName = escapeHtml(recipientName || 'there');
    const safeNumber = escapeHtml(invoiceNumber || 'your invoice');
    return renderAuthEmail({
        eyebrow: "Reminder",
        title: `Invoice <span class="highlight">Pending</span>`,
        body: `<p class="text">Hello ${safeName}, this is a friendly reminder that <strong>${safeNumber}</strong> still requires your billing information. You have <strong>${daysLeft} day${daysLeft !== 1 ? 's' : ''}</strong> left to complete the form.</p>`,
        ctaLabel: "Complete Invoice Now",
        ctaLink: null,
        footerNote: `<p class="text" style="font-size:12px; color:#6b7280;">If you've already completed this form, please disregard this message. If the link has expired, please contact us to request a new one.</p>`,
    });
}
