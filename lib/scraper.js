...
const SCRAPE_HINT_WAIT_TIMEOUT_MS = 2500;
const SCRAPE_RETRY_WAIT_MS = 1800;
...
export async function scrapeSpotifyStats(spotifyUrl, existingBrowser = null) {
    ...
    try {
        ...
        const extractStats = async () => page.evaluate(() => {
            const result = {
                name: null,
        ...
        // Prefer condition-based wait; fallback to a short retry only when extraction is empty.
        await page.waitForFunction(
            () => !!document.querySelector('script[type="text/plain"], h1'),
            { timeout: SCRAPE_HINT_WAIT_TIMEOUT_MS }
        ).catch(() => null);

        let data = await extractStats();
        if (!data?.monthlyListeners) {
            await page.waitForTimeout(SCRAPE_RETRY_WAIT_MS);
            data = await extractStats();
        }
