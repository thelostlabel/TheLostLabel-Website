...
export async function getArtistBalanceStats({ userId, userEmail, artistId }) {
    ...
    const splitFilters = [];
    if (userId) splitFilters.push(Prisma.sql`rs."userId" = ${userId}`);
    if (userEmail) splitFilters.push(Prisma.sql`LOWER(rs."email") = LOWER(${userEmail})`);
    if (artistId) splitFilters.push(Prisma.sql`rs."artistId" = ${artistId}`);

    let splitWhere = splitFilters[0];
    for (let i = 1; i < splitFilters.length; i++) {
        splitWhere = Prisma.sql`${splitWhere} OR ${splitFilters[i]}`;
    }
    splitWhere = Prisma.sql`(${splitWhere})`;

    const adjustmentWhere = userId && artistId
        ? { OR: [{ userId }, { artistId }] }
