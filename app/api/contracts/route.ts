...
function normalizeShare(value: number | string | undefined | null, defaultValue: number): number | null {
...
function normalizeSplitPercentage(value: number | string | undefined | null): number | null {
    const parsed = Number(value);
    if (!Number.isFinite(parsed)) return null;
...
function validateAndNormalizeSplits(splits: SplitInput[] | null | undefined): { normalized?: NormalizedSplit[] | null; error?: string } {
...
async function findArtistByIdentity({ artistId, userId, name, email }: FindArtistParams) {
    if (artistId) {
    ...
    const candidates = await prisma.artist.findMany({
        where: {
            OR: [
                ...
                name ? { name: { equals: name, mode: "insensitive" as const } } : undefined
...
interface NormalizeParticipantsParams {
    artistId?: string;
    userId?: string;
...
async function normalizeContractParticipants({ artistId, userId, primaryArtistName, primaryArtistEmail, splits }: NormalizeParticipantsParams) {
...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        const splitValidation = validateAndNormalizeSplits(normalizedSplits as SplitInput[] | null | undefined);
