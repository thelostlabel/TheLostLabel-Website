    ...
    try {
                ...
                await tx.contract.create({
                    data: {
                        ...
                        splits: splits && Array.isArray(splits) ? {
                            create: splits.filter((s: { name?: string; percentage?: string }) => s.name && s.percentage).map((s: { name: string; email?: string; percentage: string; userId?: string; artistId?: string }) => ({
                                name: s.name,
                                email: s.email || null,
        ...
        if (notifiedStatus && ["approved", "rejected", "contract_sent"].includes(notifiedStatus)) {
            ...
            try {
                ...
                const statusMeta: Record<string, { color: number; description: string }> = {
