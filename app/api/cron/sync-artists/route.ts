...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        let updatedCount = 0;
        const results: Array<{ name: string | null; status: string; error?: string }> = [];
        ...
        for (const artist of artistsToSync) {
            try {
                ...
                console.error(`[Cron-Sync] Error for ${artist.name}:`, errMessage);
                results.push({ name: artist.name, status: 'error', error: errMessage });
            }
        }
