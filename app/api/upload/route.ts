...
const isAllowedSignature = (ext: string, buffer: Buffer): boolean => {
...
async function writeStreamedFile({ file, filepath, ext, maxBytes }: WriteStreamedFileParams): Promise<number> {
...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        const uploadedFiles: Array<{ filename: string; filepath: string; filesize: number }> = [];
