...
function isExpired(dateValue: string | Date | null | undefined): boolean {
    if (!dateValue) return true;
    const date = new Date(dateValue);
    return Number.isNaN(date.getTime()) || date.getTime() < Date.now();
}

function resolvePublicBaseUrl(bridgeConfig: Record<string, unknown>, req?: NextRequest): string {
...
function redirectWithCode(baseUrl: string, code: string): Response {
...
export async function GET(req: NextRequest): Promise<Response> {
    ...
    let userId: string | null = stateRow.user_id;
    if (!userId) {
    ...
    try {
        const tokenResponse = await fetch("https://discord.com/api/oauth2/token", {
            ...
            body: new URLSearchParams({
                client_id: bridgeConfig.oauthClientId as string,
                client_secret: bridgeConfig.oauthClientSecret as string,
                grant_type: "authorization_code",
                code,
                redirect_uri: bridgeConfig.oauthRedirectUri as string
            })
        });
        ...
        if (error && typeof error === "object" && "code" in error && (error as { code: string }).code === "DISCORD_ACCOUNT_ALREADY_LINKED") {
