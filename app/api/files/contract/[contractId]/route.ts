...
interface ContractData {
  ...
  pdfUrl?: string | null;
  createdAt?: Date | null;
  artist?: { userId?: string | null; name?: string | null } | null;
  user?: { stageName?: string | null; legalName?: string | null; fullName?: string | null; phoneNumber?: string | null; address?: string | null } | null;
  release?: { name?: string | null; type?: string | null } | null;
  splits?: Array<{
    userId?: string | null;
    email?: string | null;
    name?: string | null;
    ...
    address?: string | null;
    user?: { id?: string; email?: string | null; fullName?: string | null; legalName?: string | null; phoneNumber?: string | null; address?: string | null } | null;
    artist?: { user?: { id?: string; fullName?: string | null; legalName?: string | null; phoneNumber?: string | null; address?: string | null } | null } | null;
  }>;
}

async function buildLegacyGeneratedContractPdf(contract: ContractData, { includeContacts = true } = {}): Promise<Buffer> {
  ...
  const drawLine = (text: string, opts: { size?: number; bold?: boolean; color?: ReturnType<typeof rgb>; gap?: number } = {}) => {
  ...
  let featuredArtists: Array<{ name?: string; role?: string; percentage?: number; email?: string; legalName?: string; phoneNumber?: string; address?: string; user?: { legalName?: string; fullName?: string; phoneNumber?: string; address?: string }; artist?: { user?: { legalName?: string; fullName?: string; phoneNumber?: string; address?: string } } }> = [];
  try {
  ...
  drawLine("Label Representative: Can Ahmet Gunduz", { size: 10, gap: 18 });
  drawLine(`Generated At: ${new Date().toISOString()}`, { size: 9, gap: 14, color: rgb(0.4, 0.4, 0.4) });

  return Buffer.from(await pdfDoc.save());
}

async function buildGeneratedContractPdf(contract: ContractData, options: { includeContacts?: boolean } = {}): Promise<Buffer> {
  const { details } = extractContractMetaAndNotes(contract.notes || "");
  let featuredArtists: Array<{ name?: string }> = [];
  try {
    featuredArtists = contract.featuredArtists ? JSON.parse(contract.featuredArtists) : [];
  } catch {
    featuredArtists = [];
  ...
  try {
    ...
    const singleLine = (value: string | null | undefined, max = 90): string => {
      const clean = String(value || "").replace(/\s+/g, " ").trim();
      if (!clean) return "-";
      return clean.length > max ? `${clean.slice(0, max - 1)}...` : clean;
    };

    const wrapLines = (value: string | null | undefined, maxPerLine = 48, maxLines = 2): string[] => {
      const clean = String(value || "").replace(/\s+/g, " ").trim();
      if (!clean) return [];
    ...
    if (pages.length >= 7) {
      ...
      const drawSchedule = (text: string, x: number, y: number, size = 9, bold = false) => {
    ...
    console.warn("Contract template not found/readable, falling back to legacy PDF:", error instanceof Error ? error.message : "Unknown error");
    return buildLegacyGeneratedContractPdf(contract, options);
  }
}

const resolveStoragePath = (filepath: string | null | undefined): string[] => {
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ contractId?: string }> }) {
  ...
  try {
    ...
    console.error("Contract file read error:", error instanceof Error ? error.message : "Unknown error");
