...
function isAllowedDiscordHost(url: string): boolean {
    try {
...
function inferFileName(body: Record<string, unknown>): string {
    const explicit = (body?.fileName || body?.filename) as string | undefined;
...
async function downloadDiscordFile(fileUrl: string): Promise<DownloadedFile> {
...
async function persistDemoFile(filename: string, buffer: Buffer): Promise<PersistedFile> {
...
export async function POST(req: NextRequest): Promise<Response> {
    ...
    try {
        ...
        let fileRecord: PersistedFile | null = null;
        if (fileUrl) {
        ...
        return discordInternalJson({ error: error instanceof Error ? error.message : "Failed to submit demo." }, 500);
    }
}
