...
interface ReleaseRow {
    ...
    releaseArtists: { artistId: string; name: string }[];
}

async function getRelease(rawSlug: string): Promise<ReleaseRow | null> {
...
function getArtistNames(release: ReleaseRow): string {
...
function getReleaseYear(release: ReleaseRow): string | null {
    if (!release.releaseDate) return null;
    const match = release.releaseDate.match(/\d{4}/);
    return match ? match[0] : null;
}

function getReleaseTypeLabel(type: string | null): string {
        ...
        <>
            ...
            >
                ...
                <p>
                    ...
                    {release.totalTracks && release.totalTracks > 1 ? ` ${release.totalTracks} tracks.` : ""}
