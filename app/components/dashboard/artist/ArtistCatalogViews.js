...
export function DemosView({ demos, onNavigate, onDelete, shared }) {
  ...
  const handlePlay = (demo) => {
    ...
    if (playingId === demo.id && audioRef.current) {
      ...
      {demos.length === 0 ? (
        ...
        <div style={{ display: "grid", gap: "15px" }}>
          {demos.map((demo) => (
            <div
              key={demo.id}
              style={{ ...glassStyle, padding: "25px", transition: "transform 0.2s" }}
              onMouseEnter={(event) => {
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {demo.files && demo.files.length > 0 && (
                    <p style={{ fontSize: "12px", color: DASHBOARD_THEME.muted, marginTop: "5px" }}>
                      Files attached: {demo.files.length}
                    </p>
                  )}
                  ...
                  <div style={{ display: "flex", gap: "10px", marginTop: "16px", flexWrap: "wrap" }}>
                    {getDemoAudioAvailable(demo) && (
                      <button
                        ...
                        style={{
                          ...btnStyle,
                          background: playingId === demo.id ? "rgba(255,255,255,0.15)" : "rgba(255,255,255,0.08)",
                          color: "#fff",
                    ...
                    <Link
                      ...
                      style={{
                        ...btnStyle,
                        background: DASHBOARD_THEME.accent,
                        color: "#000",
                        border: "none",
                        padding: "8px 18px",
                        fontSize: "10px",
