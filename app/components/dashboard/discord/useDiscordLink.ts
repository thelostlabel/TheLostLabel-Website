...
const EMPTY_DISCORD_LINK: DiscordLink = {
    linked: false,
    discordUserId: null,
...
async function readDiscordLink(userId: string | undefined): Promise<DiscordLink> {
    if (!userId) {
...
export function useDiscordLink(userId: string | undefined) {
    const [discordLink, setDiscordLink] = useState<DiscordLink>(EMPTY_DISCORD_LINK);
