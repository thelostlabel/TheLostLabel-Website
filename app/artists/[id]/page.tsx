...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
    ...
    const image = (artist as SpotifyArtist | null)?.images?.[0]?.url || null;
    ...
    return {
        ...
        openGraph: {
            ...
            type: 'profile',
            siteName: LABEL_NAME,
            images: image ? [{ url: image, width: 640, height: 640, alt: name ?? undefined }] : [{ url: '/logo.png' }],
        },
        twitter: {
...
export default async function ArtistDetailPage({ params }: PageProps) {
    ...
    const artist = artistDetails?.[0] as (SpotifyArtist & { monthlyListeners?: number; dbInfo?: typeof dbArtist; genres?: string[] }) | undefined;
    ...
    const groupedReleases: Record<string, GroupedRelease> = {};
    ...
    // Sort by date
    releases.sort((a, b) => new Date(b.release_date as string).getTime() - new Date(a.release_date as string).getTime());
                        ...
                        <ul>
                            {releases.map((r) => (
                                <li key={r.id}>
                                    ...
                                    {r.release_date ? ` (${new Date(r.release_date as string).getFullYear()})` : ""}
                                    {r.type ? ` — ${r.type}` : ""}
                                </li>
                            ))}
                        </ul>
