...
export default function ArtistDetailClient({ artist, releases }: ArtistDetailClientProps) {
    ...
    useEffect(() => {
        return scrollY.on('change', (v: number) => setScrolledPastHero(v > (heroRef.current?.offsetHeight || 500)));
    }, [scrollY]);
    ...
    const handlePlayRelease = useCallback((release: ArtistRelease) => {
        if (release.preview_url || release.previewUrl) {
            playTrack({
                ...
                previewUrl: release.preview_url || release.previewUrl || undefined,
            });
        }
    }, [playTrack, artist]);
                        ...
                        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                            {artist.external_urls?.spotify && (
                                <a href={artist.external_urls.spotify} target="_blank" rel="noopener noreferrer"
                                    ...
                                    onMouseEnter={(e: MouseEvent<HTMLAnchorElement>) => { e.currentTarget.style.transform='translateY(-2px)'; e.currentTarget.style.boxShadow='0 10px 30px rgba(255,255,255,0.2)'; }}
                                    onMouseLeave={(e: MouseEvent<HTMLAnchorElement>) => { e.currentTarget.style.transform='none'; e.currentTarget.style.boxShadow='none'; }}
                            ...
                            <Link href="/artists"
                                ...
                                onMouseEnter={(e: MouseEvent<HTMLAnchorElement>) => { e.currentTarget.style.background='rgba(255,255,255,0.06)'; e.currentTarget.style.color='#fff'; }}
                                onMouseLeave={(e: MouseEvent<HTMLAnchorElement>) => { e.currentTarget.style.background='transparent'; e.currentTarget.style.color='rgba(255,255,255,0.5)'; }}
                            >ALL ARTISTS</Link>
                        </div>
                    </motion.div>
                </div>
                    ...
                    <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-60px' }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 20 }}>
                            {releases.map((release, i) => {
                                ...
                                const releaseYear = release.release_date ? new Date(release.release_date as string).getFullYear() : null;
                                return (
                                    <motion.div
                                        key={release.id}
                                        initial={{ opacity: 0, y: 20 }}
                                    ...
                                    >
                                        <Link href={`/releases/${rSlug}`} style={{ textDecoration: 'none', display: 'block' }}>
                                            <div
                                                ...
                                                onMouseEnter={(e: MouseEvent<HTMLDivElement>) => { e.currentTarget.style.transform='translateY(-5px)'; e.currentTarget.style.boxShadow=`0 20px 50px rgba(0,0,0,0.7), 0 0 30px rgba(${accentColor},0.12)`; }}
